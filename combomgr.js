...
TrieNode.prototype.add = function(sequence, leafData, options) {
    ...
    if (index === sequence.length) {
        this.data = leafData;
        if (options.preserve) {
            this._metadata.preserve = options.preserve || false;
        }
...
TrieNode.prototype.get = function(sequence, index) {
    ...
    if (index === sequence.length) {
        ...
        var token = sequence[index];
        var child = this.children[token] || undefined;
...
var keyHeld = {};
var baseTrie = new TrieNode("blah");
var keyBuffer = [];

baseTrie.add(["a", "b", "a", "c"], function() { console.log("hi"); });
baseTrie.add(["q", "w", "e"], function() { console.log("qwe!"); });

function advanceBuffer(key) {
    for (var i = 0; i < keyBuffer.length; i++) {
        var trie = keyBuffer[i];
    ...
    var newTrie = baseTrie.get([key]);
    if (typeof newTrie !== 'undefined') {
        keyBuffer.push(newTrie);
    }
}
